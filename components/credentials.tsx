...
export function Credentials() {
  return (
    <Section id="credentials" label="03" title="Credentials">
      {/* Education */}
      <div className="mb-10">
        <h3 className="mb-3 border-b border-border pb-2 font-mono text-[0.75rem] font-medium uppercase tracking-[0.1em] text-forest">
          Education
        </h3>
        <div className="border border-border bg-parchment-dark p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h4 className="mb-1 font-serif text-[1.25rem] font-semibold text-ink">
            ...
            <div className="flex flex-wrap items-start gap-2">
              <span className="border border-border-strong px-3 py-1.5 font-mono text-[0.7rem] tracking-[0.03em] text-muted">
                2024 – 2027
              </span>
              <span className="border border-forest px-3 py-1.5 font-mono text-[0.7rem] tracking-[0.03em] text-forest">
                GPA 4.00
              </span>
              <span className="border border-border-strong px-3 py-1.5 font-mono text-[0.7rem] tracking-[0.03em] text-muted">
                3-Year Accelerated
      ...
      <div className="mb-10">
        ...
        <p className="mb-4 text-[0.8rem] italic text-muted-foreground">
        ...
        <ul className="m-0 grid list-none gap-2 p-0 sm:grid-cols-2">
          {graduateCourses.map((course) => (
            <li
              key={course.code}
              className="border-b border-border py-1.5 text-[0.9rem]"
            >
              <span className="mr-3 font-mono text-[0.8rem] text-walnut">
                {course.code}
              </span>
              {course.name}
            </li>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {involvement.map((item) => (
            <span
              key={item}
              className="border border-border bg-parchment-dark px-4 py-2 text-[0.85rem] text-muted"
            >
              {item}
            </span>
          ))}
